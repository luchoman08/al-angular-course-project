...
export class SearchInputComponent implements OnInit {
  ...
  results$: Observable<
    ResultsInterface<
      MovieCombinedInterface | PersonCombinedInterface | TvCombinedInterface
    >
  >;
  constructor(private searchService: SearchService) {
    ...
    this.results$ = this.searchService
      .searchCombined(this.stateCtrl.valueChanges)
      .pipe(
        map((data) => {
          if (data) {
            /** Does not support tv  */
            data.results = data.results.filter(
              (result) => result.media_type != this.MEDIA_TYPE.TV
            );
          }
          return data;
        })
