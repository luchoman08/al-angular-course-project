...
export class ZoomImageDirective {
  ...
  private isTheLastOfMoreThanTwo(): boolean {
    return (
      this.index === this.number_of_elements - 1 && this.number_of_elements > 2
    );
  }
  @HostListener("mouseover") onmouseover() {
    ...
    if (this.index && this.number_of_elements) {
      if (this.isTheFirstOneOfMoreThanTree()) {
        this.renderer.setStyle(this.el.nativeElement, "margin-top", "10px");
        this.renderer.setStyle(
          this.el.nativeElement,
          "transform",
          "scale(2.0)"
      ...
      this.renderer.setStyle(this.el.nativeElement, "transform", "scale(2.5)");
    }
  }
  @HostListener("mouseout") onmousedown() {
    ...
    if (this.position_hover) {
      this.renderer.setStyle(this.el.nativeElement, "position", "initial");
    }
    if (this.index) {
      this.renderer.setStyle(this.el.nativeElement, "margin-top", "0px");
    }
  }
}
