...
export class MoviesListTableComponent implements OnInit, OnChanges {
    ...
    this.movies = this.movies.pipe(
      map((movies: Movie[]) => {
        return movies.map((movie) => {
          movie.genres = this.genreService
            .getMultiple(movie.genre_ids)
  ...
  sortData(sort: Sort) {
    ...
    this.movies = this.movies.pipe(
      map((data) =>
        data.sort((movie1: Movie, movie2: Movie) => {
          let isAsc = sort.direction == "asc";
          switch (sort.active) {
            case "title":
              return compare(movie1.title, movie2.title, isAsc);
            case "vote_average":
              return compare(+movie1.vote_average, +movie2.vote_average, isAsc);
            case "release_date":
              return compare(
                new Date(movie1.release_date),
                new Date(movie2.release_date),
                isAsc
              );
            case "vote_count":
              return compare(+movie1.vote_count, +movie2.vote_count, isAsc);
            default:
              return 0;
          }
        })
